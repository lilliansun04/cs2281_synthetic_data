#!/bin/sh
#SBATCH --job-name=finetuning_summarization_debug
...
dataset_prop=0.1 # how much of the dataset to use (as a fraction not a percentage)
...
unique_save_name="t5-small_"$dataset_prop"_debug_2"
...
python -u $CODE_DIR/finetuning_summarization.py $model_name $output_dir $scratch_dir $unique_save_name $dataset_prop $summarization_train_filepath $summarization_val_filepath $summarization_test_filepath $batch_size $eval_steps
echo "DONE"